...
import datetime, xlrd ,time, sys , os ,csv,unittest
...
class Window:
    ...
    def ensure_dir(self,nameFolder):
        ...
        if os.path.exists(newpath):
            ...
            os.chdir(newpath)
            A.setFilename(self.filename)
            workbook = xlrd.open_workbook(self.filename)
            self.sheed = workbook.sheet_by_index(0)
            self.listHead = A.Headlist()
            print (self.listHead)

            A.setlist()

            self.listTypeMeasures = A.getlistTypeMeasures()
            self.listTypeMeasuresLocation = A.getlistTypeMeasuresLocation()

            self.listTypeDimensions = A.getlistTypeDimensions()
            self.listTypeDimensionsLocation = A.getlistTypeDimensionsLocation()

            self.listTypeDate = A.getlistTypeDate()
            self.listTypeDateLocation = A.getlistTypeDateLocation()

            print (self.listTypeDimensions)
            print (self.listTypeDimensionsLocation)

            self.listTypeInDimensions = A.getlistTypeInDimensions()
            self.listSaveGUI = A.getlistSaveGUI()
            print ("Wait")
            ...
            for countDate in range(len(self.listTypeDate)):
                ...
                A.setYearlist(dateHead)
                yearlist = A.getlistyear()
...
HeadInlist = B.listINheadN(Headlist[0])
show = B.listDimensionsFunc(Headlist[0],HeadInlist[0])

class TestStringMethods(unittest.TestCase):

    def test_Head(self):
        self.assertEquals((Headlist),['Country', 'Postal Code', 'Market', 'Region', 'Product ID', 'Category', 'Sub-Category', 'Product Name', 'Sales', 'Quantity', 'Discount', 'Profit', 'Shipping Cost', 'Order Priority', 'Order Date', 'Ship Date'])

    def test_searchHead(self):
        self.assertEquals(B.searchHead('Country'),1)

    def test_InHead(self):
        self.assertEquals(B.listINheadN(Headlist[0]),['Asia','Europe'])
